...
extern void send_log(const char *username, const char *operation, const char *timestamp);
...
void *handle_client(void *socket_desc) {
    ...
    if (strcmp(operation, "REGISTER") == 0) {
        ...
        // RPC service call
        send_log(username, "REGISTER", datetime);
        ...
        // RPC service call
        send_log(username, "UNREGISTER", datetime);
        ...
        // RPC service call
        send_log(username, "CONNECT", datetime);
        ...
        // RPC service call
        send_log(username, "DISCONNECT", datetime);
        ...
        char operation_msg[256];
        snprintf(operation_msg, sizeof(operation_msg), "PUBLISH %.240s", filename);

        // RPC service call
        send_log(username, operation_msg, datetime);
        ...
        // Create operation message
        char operation_msg[256];
        snprintf(operation_msg, sizeof(operation_msg), "DELETE %.241s", filename);
        ...
        // RPC service call
        send_log(username, "LIST_USERS", datetime);
        ...
        // RPC service call
        send_log(username, "LIST_CONTENT", datetime);
