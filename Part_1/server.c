...
#define MAX_STRING 256   // Maximum size of strings
#define INITIAL_FILES 10    // Initial capacity of files array per user

// Structure to store published files information
typedef struct {
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        *new_sock = client_socket;
        ...
        // Detach thread so it can clean itself automatically
...
void remove_user(char *username) {
    ...
    if (index != -1) {
        // Move the last user to the position of the removed one
...
int remove_file(int user_index, char *filename) {
    ...
    for (int i = 0; i < users[user_index].num_files; i++) {
        if (strcmp(users[user_index].files[i].filename, filename) == 0) {
            // Move the last file to the position of the removed one
